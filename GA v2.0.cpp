...
//  Purpose:
//
//    printVector prints the content of the 
//
//  Discussion:
//
...
//  Parameters:
//
//    PopulationSize is the starting population of the Algorythm.
//
//    HiddenNeurons is the amount of Hidden Neurons of a Neural Net.
//
//    LearningRate is the Learning Rate of a Neural Net.                          
//
//    MomentumConstant is the Momentum Constant of a Neural Net.
//
//    ProbXOver is the probability of crossover in a Generation Change.                          
//
//    ProbMutation is the probability of mutation in a Generation Change.
//
{
	cout << vectorName << ": ";
...
//
{
  ...
  if(argc != 7)
  {
   	cerr << "Usage: " << argv[0] << " PopulationSize " << "HiddenNeurons " <<
            "LearningRateEta " << "MomentumConstantAlpha " <<
            "Probability Of Crossover " << "Probability Of Mutation" << endl;
   	return 1;
  }
  
  bool exit_val = false;
  cout << "Welcome to AddaNN!" << endl;
  unsigned PopulationSize = atoi(argv[1]);
  unsigned HiddenNeurons = atoi(argv[2]);
  double LearningRate = atof(argv[3]);
  double MomentumConstant = atof(argv[4]);
  double ProbXOver = atof(argv[5]);
  double ProbMutation = atof(argv[6]);
